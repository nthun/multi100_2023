...
wb_lmer <- 
  ...
  mutate(model = map(data, 
                     ~lmer(
                       wellbeing ~ age + age2 + married + edu + paredu + 
...
tab_model(wb_lmer$model[[1]], wb_lmer$model[[2]],
...
```{r}
test1 <- 
  lmer(wellbeing ~ gender + age_ctd + age_ctd2 + married + edu + paredu + 
                   rel + pdwrk + brncntr + sclmeet + 
                   par_stat * childless_disapp_std +
                   (1 | cntry),
       data = wb)
...
tab_model(test1, 
          ...
          show.reflvl = TRUE, prefix.labels = "v",
          order.terms = 
          )
